...
export default function TypingTest({ forceDuration = null, forcePreset = null } = {}) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    let initDuration = forceDuration || 30;
    ...
    if (forcePreset && forcePreset.words && forcePreset.words.length > 0) {
      setCustomWords(forcePreset.words);
      setCustomFileName(forcePreset.label || "Preset");
      setWords(generateFromList(300, forcePreset.words));
    } else {
